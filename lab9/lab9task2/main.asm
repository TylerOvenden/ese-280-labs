...
;*
;* DESCRIPTION
;* Uses positive edge triggered pin change interrupts to count the number
;* of times each of two pushbuttons is pressed. Pushbutton 1 is connected
;* to PE0 and pushbutton 2 is connected to PE2. The counts are stored in
;* two byte memory variable.
;*
;* VERSION HISTORY
...
start:
	...
	ldi YH, HIGH(PORTA_PIN0CTRL)     ;creates pointer set to PINOCTRL
	ldi YL, LOW(PORTA_PIN0CTRL)      ;
...
push r16
    ...
    ldi r16, PORT_INT2_bm	;clear IRQ flag for PE0
	sts PORTE_INTFLAGS, r16
	pop r16
	out CPU_SREG, r16
	pop r16
...
//	ori r16, 0xF4 
	st Y+, r16                    ;stores results & increases pointer
